...
class Config:
    ...
    def __init__(self, config_file):
        ...
        self.owner_id = config.get('Permissions', '716597350625181706', fallback=ConfigDefaults.owner_id)
        self.dev_ids = config.get('Permissions', 'auto', fallback=ConfigDefaults.dev_ids)
        self.bot_exception_ids = config.get("Permissions", "BotExceptionIDs", fallback=ConfigDefaults.bot_exception_ids)

        self.command_prefix = config.get('Chat', '4', fallback=ConfigDefaults.command_prefix)
